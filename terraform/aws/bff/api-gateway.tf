...
resource "aws_apigatewayv2_route" "soap_gaps_chat" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/soap-gaps-chat"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
...
resource "aws_apigatewayv2_route" "material_candidates_promote" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/material-candidates/{candidateId}/promote-to-material"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
...
resource "aws_apigatewayv2_route" "rubrics_active" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "PATCH /api/rubrics/{rubricId}/active"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "soap_knowledge_base_list" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "GET /api/soap-knowledge-base"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "soap_knowledge_base_create" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/soap-knowledge-base"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "soap_knowledge_base_status" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "PATCH /api/soap-knowledge-base/{knowledgeBaseId}/status"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "soap_knowledge_base_items_list" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "GET /api/soap-knowledge-base/{knowledgeBaseId}/items"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "soap_knowledge_base_items_create" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/soap-knowledge-base/{knowledgeBaseId}/items"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "soap_knowledge_base_item_active" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "PATCH /api/soap-knowledge-base-items/{itemId}/active"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "prompt_templates_list" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "GET /api/prompt-templates"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "prompt_templates_create" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/prompt-templates"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
...
resource "aws_apigatewayv2_route" "exercise_cases_list" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "GET /api/exercise-cases"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "exercise_cases_detail" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "GET /api/exercise-cases/{caseId}"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "exercise_cases_create" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/exercise-cases"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "exercise_attempts_create" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/exercise-attempts"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "exercise_attempts_list" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "GET /api/exercise-attempts"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "exercise_attempts_reveal_followup" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "PATCH /api/exercise-attempts/{attemptId}/reveal-followup"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "exercise_attempts_draft_answers" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "PATCH /api/exercise-attempts/{attemptId}/draft-answers"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "exercise_attempts_submit" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/exercise-attempts/{attemptId}/submit"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}

resource "aws_apigatewayv2_route" "instructor_comments_create" {
  api_id               = aws_apigatewayv2_api.this.id
  authorization_scopes = var.jwt_authorization_scopes
  authorization_type   = "JWT"
  authorizer_id        = aws_apigatewayv2_authorizer.jwt.id
  route_key            = "POST /api/instructor-comments"
  target               = "integrations/${aws_apigatewayv2_integration.lambda.id}"
}
