    ...
    variables = merge(
      {
        ...
        VOICE_CAPTURE_BUCKET              = aws_s3_bucket.voice_capture.bucket
        VOICE_CAPTURE_LANGUAGE_CODE       = var.voice_capture_language_code
        VOICE_CAPTURE_TRANSCRIBE_ROLE_ARN = aws_iam_role.voice_capture_transcribe.arn
        WS_URL_EXPIRES_SECONDS            = tostring(var.ws_url_expires_seconds)
      },
      var.enable_training_data_store ? {
