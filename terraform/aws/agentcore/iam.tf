...
data "aws_iam_policy_document" "runtime" {
  ...
  statement {
    ...
    resources = concat(
      ...
      var.enable_law_hierarchical_comparison ? [aws_bedrockagent_knowledge_base.law_hierarchical[0].arn] : [],
...
data "aws_iam_policy_document" "kb_service" {
  ...
  dynamic "statement" {
    for_each = var.enable_law_hierarchical_comparison ? [1] : []

    content {
      sid       = "AccessLawHierarchicalOpenSearchServerless"
      effect    = "Allow"
      actions   = ["aoss:APIAccessAll"]
      resources = [aws_opensearchserverless_collection.law_hierarchical[0].arn]
    }
  }
}
