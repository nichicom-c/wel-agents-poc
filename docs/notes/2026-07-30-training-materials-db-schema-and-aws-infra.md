...
- **その後に撤去した機能**: issue #9（新人保健師向け演習）は全体を撤去済み（`exercise_*` テーブル・API・`exercise_feedback_agent` とも無い）。issue #10 のうち **SOAP マッピング**（`soap_mapping_versions` / `soap_record_versions.soap_mapping_version_id` / `GET|POST /api/soap-mapping-versions` / Admin の「SOAP マッピング」タブ）も撤去済みで、稼働中 DB からも drop 済み（migration は `0001_init.sql` に畳み込み）。**品質指標**（`quality_metrics_definitions` / `quality_metric_key` / `GET /api/quality-metrics` / Admin の「品質指標」タブ）も撤去済み — 下記のとおり集計 view も目標値設定も作らない read-only の定義マスタのままで、参照する機能が現れなかった（稼働中 DB からも drop 済みで、migration は `0001_init.sql` / `0002_seed_masters.sql` に畳み込み）。**必須・推奨項目**（`required_recommended_items` / `requirement_level` / `GET|POST /api/required-items` / Admin の「必須・推奨項目」タブ）も撤去済み — 記録種別×分野ごとの必須/推奨項目マスタを登録できるだけで、不足確認（issue #6 の `missing_required` / `missing_recommended` は LLM 判定でこのマスタを見ない）も集計（`aggregation_category` の参照先だった品質指標は撤去済み）もこのマスタを読まず、seed も無いまま空だった（稼働中 DB からも drop 済みで、migration は `0001_init.sql` に畳み込み）。**参照知識**（`reference_knowledge` / `material_reference_knowledge` / `rubric_reference_knowledge` / `reference_knowledge_source_type` / `GET /api/reference-knowledge` / Admin の「参照知識」タブ）も 2026-09-17 に撤去済み — read-only の一覧しか無く、作成 UI も seed も無いまま空で、vector KB 上のドキュメントへの参照を人手で登録する経路が現れなかった。あわせて、コードから一度も参照されていなかった `professional_comment_revisions`（コメント編集 API 自体が無い）と `soap_record_versions.recording_id`（insert 文にも select 文にも無い）も撤去した。`app_users.roles` は同じく未使用だが、Cognito Group ミラーの設計意図を残すため列だけ維持している（`0001_init.sql` のコメントに未実装であることを明記）。
- **migration 運用のルール化（2026-09-17）**: 「`0001_init.sql` へ畳み込む」方針を維持したうえで、**差分ファイルを積まず DB を作り直す**ことを明文化した。migration ファイルは `0001`〜`0003` の3本に固定し、スキーマを変えたら `0001_init.sql` を直接書き換えて `drop schema public cascade` → 再適用する。「`0001`〜`0003` を初回適用しただけの状態」以外を存在させないことで、`0001_init.sql` が常にスキーマの正になり、環境ごとの適用履歴を追う必要が無くなる（PoC で捨てて良いデータしか無いことが前提。運用データを持つようになったら冪等な差分ファイルを積む方式へ切り替える）。作り直しの取りこぼしを検出するため、migration ランナーは適用済みだが `migrations/` に実体が無いファイル名を `[WARNING]` で出す（警告が出たらその DB は作り直せていない）。手順は `terraform/aws/bff/README.md` の「Training Data Store」節を参照。

以下の本文（ER 図・DDL・画面遷移）はいずれも撤去前の検討時点の記述として残している。スキーマの正は `terraform/aws/bff/migrations/0001_init.sql`。
...
- **スキーマの細部**が実装時に変わっている: enum 相当の列は提案時の `text + check` ではなく Postgres の native `enum` 型（`material_type` / `publication_status` 等）で実装、`materials.material_type` の値は `exercise_case` ではなく `teaching_case` / `comment_derived_note` / `reference_summary`、`specialty_id` / `learning_theme_id` / `difficulty_id` / `record_type` は `uuid` FK ではなくブラウザ表示用固定文字列を id にした `text` FK（`specialties.id` 等）。正確な列定義は `terraform/aws/bff/migrations/0001_init.sql` を正とする。
