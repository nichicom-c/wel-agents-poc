...
- **その後に撤去した機能**: issue #9（新人保健師向け演習）は全体を撤去済み（`exercise_*` テーブル・API・`exercise_feedback_agent` とも無い）。issue #10 のうち **SOAP マッピング**（`soap_mapping_versions` / `soap_record_versions.soap_mapping_version_id` / `GET|POST /api/soap-mapping-versions` / Admin の「SOAP マッピング」タブ）も撤去済みで、稼働中 DB からも drop 済み（migration は `0001_init.sql` に畳み込み）。**品質指標**（`quality_metrics_definitions` / `quality_metric_key` / `GET /api/quality-metrics` / Admin の「品質指標」タブ）も撤去済み — 下記のとおり集計 view も目標値設定も作らない read-only の定義マスタのままで、参照する機能が現れなかった（稼働中 DB からも drop 済みで、migration は `0001_init.sql` / `0002_seed_masters.sql` に畳み込み）。**必須・推奨項目**（`required_recommended_items` / `requirement_level` / `GET|POST /api/required-items` / Admin の「必須・推奨項目」タブ）も撤去済み — 記録種別×分野ごとの必須/推奨項目マスタを登録できるだけで、不足確認（issue #6 の `missing_required` / `missing_recommended` は LLM 判定でこのマスタを見ない）も集計（`aggregation_category` の参照先だった品質指標は撤去済み）もこのマスタを読まず、seed も無いまま空だった（稼働中 DB からも drop 済みで、migration は `0001_init.sql` に畳み込み）。以下の本文（ER 図・DDL・画面遷移）はいずれも撤去前の検討時点の記述として残している。
