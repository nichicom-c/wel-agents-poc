...
- **その後に撤去した機能**: issue #9（新人保健師向け演習）は全体を撤去済み（`exercise_*` テーブル・API・`exercise_feedback_agent` とも無い）。issue #10 のうち **SOAP マッピング**（`soap_mapping_versions` / `soap_record_versions.soap_mapping_version_id` / `GET|POST /api/soap-mapping-versions` / Admin の「SOAP マッピング」タブ）も撤去済みで、稼働中 DB からも drop 済み（migration は `0001_init.sql` に畳み込み）。以下の本文（ER 図・DDL・画面遷移）はいずれも撤去前の検討時点の記述として残している。
