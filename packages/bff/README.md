...
| File | Summary |
| --- | --- |
| `adapters/dev-server.ts` | `Bun.serve` で `/ping`、`/api/ws-url`、`/api/sessions`、`/api/dev-info`、`/api/soap-draft`、`/api/soap-gaps`、`/api/soap-records*`、`/api/professional-comments`、`/api/material-candidates*`、`/api/training-data-cluster*`、`/api/voice-recordings*`、fallback `/api/chat` を公開します。`/api/ws-url` は local `/ws` URL を返し、`/api/sessions` は configured Memory ID がある時だけ AWS `ListSessions` を呼び、`/api/dev-info` は local Runtime `/ping` を確認し、`/api/soap-draft`・`/api/soap-gaps`・`/api/chat` は local AgentCore Runtime へ `fetch` で forward し、`/api/soap-records*`・`/api/professional-comments`・`/api/material-candidates*` は Aurora を RDS Data API で直接呼び、`/api/training-data-cluster*` は同じ Aurora cluster を RDS control-plane（`DescribeDBClusters`/`StartDBCluster`）で直接呼び、`/api/voice-recordings*` は S3 / Amazon Transcribe を直接呼びます。idleTimeout はデフォルトの Bun 10秒では AgentCore の生成待ちに足りないため 60秒へ明示的に延ばしています。 |
| `adapters/lambda.ts` | API Gateway event を受け、`/api/ws-url`、`/api/sessions`、`/api/dev-info`、`/api/soap-records*`、`/api/professional-comments`、`/api/material-candidates*`、`/api/training-data-cluster*` は JWT claims から認証 context を作って application handler に委譲し、`/api/soap-draft`・`/api/soap-gaps`・fallback `/api/chat` は AgentCore Runtime SDK client を注入してそれぞれの handler を呼び、`/api/soap-records*`・`/api/professional-comments`・`/api/material-candidates*` は RDS Data API 呼び出しを注入して handler を呼び、`/api/training-data-cluster*` は RDS control-plane 呼び出しを注入して handler を呼び、`/api/voice-recordings*` は S3 / Transcribe 呼び出しを注入して handler を呼びます。 |
...
| `application/handle-masters-request.ts` | `GET /api/masters` の routing、Training Data Store 未設定時の 503、authContext 未設定時の 401、store 例外の 502 変換を担います（read-only）。画面が分野・学習テーマ・難易度・却下理由の id/label を固定値で持たないための配信元です。 |
| `application/handle-training-data-cluster-request.ts` | `GET /api/training-data-cluster`（現在の cluster status）+ `POST /api/training-data-cluster/start`（起動要求）の routing、cluster ARN 未設定時の 503、authContext 未設定時の 401、RDS control-plane 例外の 502 変換を担います。 |
...
| `contracts/admin.ts` | 管理画面（issue #10）の教材 / 評価ルーブリックの定数・型ガード・型を定義します。 |
...
| `infra/master-store.ts` | `specialties` / `learning_themes` / `difficulty_levels` / `rejection_reason_codes` の read のみを担います。難易度は `order_no`、他は id/code で並び順を固定します。 |
| `infra/training-data-cluster-control.ts` | `@aws-sdk/client-rds` の `DescribeDBClustersCommand`/`StartDBClusterCommand` を組み立て、Training Data Store cluster の status 取得・起動を担います。既に起動中/起動済みの `InvalidDBClusterStateFault` は無視し、最新 status を返します。 |
...
- `/api/training-data-cluster*` の request/response contract や RDS control-plane 呼び出しを変える場合は `application/handle-training-data-cluster-request.ts`、`infra/training-data-cluster-control.ts` を更新し、`adapters/lambda.ts` / `adapters/dev-server.ts` の options 組み立て、`packages/workbench` の `src/features/admin/api/admin.ts`（Admin 画面の `TrainingDataClusterPanel` から呼ぶ）・`vite.config.ts` の proxy、`terraform/aws/bff`（`api-gateway.tf` の route、`iam.tf` の `rds:StartDBCluster`/`rds:DescribeDBClusters` 権限）、`terraform/aws/wel-agents-training-data-policy.json`（local dev で使う operator 権限）を合わせます。
- マスタ（分野・学習テーマ・難易度・却下理由）の項目を増減する場合は `terraform/aws/bff/migrations/0002_seed_masters.sql` だけを変えます。`GET /api/masters`（`application/handle-masters-request.ts`、`infra/master-store.ts`、`contracts/masters.ts`）が id/label をそのまま配り、`packages/workbench` は `src/features/masters/` 経由で受け取るため、画面側に固定値はありません。id は `materials` / `material_candidates` の FK なので、既存データがある状態では id を変えず label だけ直します。
- 管理画面（issue #10）の `/api/materials*` / `/api/rubrics*` の request/response contract や SQL を変える場合は `application/handle-material-request.ts` / `handle-rubric-request.ts`、`contracts/admin.ts`、`infra/material-store.ts` / `rubric-store.ts`（共通 helper は `infra/training-data-sql.ts`）を更新し、`terraform/aws/bff/migrations/*.sql` のスキーマ、`packages/workbench` の `src/features/admin/`、`terraform/aws/bff/api-gateway.tf` の route（`TRAINING_DATA_*` env は `/api/soap-records*` と共用）を合わせます。
