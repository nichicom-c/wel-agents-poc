...
/** body.candidates が SOAP 下書き候補配列らしいかを検証し、そのまま payload へ渡す形にする。 */
function candidatesField(value: unknown): unknown[] | undefined {
  if (!Array.isArray(value) || value.length === 0) {
    return undefined;
  }
  return value.every(isCandidateLike) ? value : undefined;
}
...
function isCandidateLike(value: unknown): boolean {
  ...
  return (
    ...
    SOAP_CATEGORIES.includes(record.category) &&
