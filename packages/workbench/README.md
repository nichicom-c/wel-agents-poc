...
Knowledge Review（issue #8）は SOAP 正式記録・版（`soap_records` / `soap_record_versions`）に加え、専門職コメント（`professional_comments`）と教材候補（`material_candidates` / `material_candidate_comments` / `material_candidate_status_events`）も BFF `/api/professional-comments` / `/api/material-candidates*` を実際に呼びます。「記録から探す」タブでコメントを投稿し、選択したコメントから教材候補を作成、レビュー承認者/管理者ロールで承認/却下/要修正を決定できます（却下は理由分類、要修正は補足コメントが必須）。ロール切り替えは demo 用の UI 表示制御のみで、実際の認可（Cognito Group 等）はまだ実装していません。Training（issue #9）・Admin（issue #10）は dummy データで動作します（詳細は各 `src/features/*/api/` を参照）。
